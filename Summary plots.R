...
t <- t+ theme(legend.title=element_text(size=16),legend.text=element_text(size=16),legend.position = "bottom", legend.spacing.x = unit(0.5, 'cm'))
ggsave("TavgF-Timeseries.png",t, path = FigDir, height=PlotHeight, width=PlotWidth, bg="white")

# Precip
p<-LT_plot(yrAvgs,PrcpIn,rollvar=PrcpRoll10,cols=col,yaxis="Mean annual precipitation (inches/Yr)",title="")
ggsave("PrcpIn-Timeseries.png", p, path = FigDir, height=PlotHeight, width=PlotWidth, bg="white")
...
ggsave("D.in-Timeseries.png", d, path = FigDir, height=PlotHeight, width=PlotWidth, bg="white")

# Runoff
r<-LT_plot(WBAvgs, runoff.in, rollvar=Runoff.inRoll10, cols=col,yaxis="Mean annual runoff (in/year)",title="")
r <- r+ theme(legend.title=element_text(size=16),legend.text=element_text(size=16),legend.position = "bottom", legend.spacing.x = unit(0.5, 'cm'))
ggsave("Runoff.in-Timeseries.png", r, path = FigDir, height=PlotHeight, width=PlotWidth, bg="white")
...
ggsave("SWEaccum.in-Timeseries.png", path = FigDir, height=PlotHeight, width=PlotWidth, bg="white")
...
ggsave("TavgF-PrcpIn-Timeseries.png", g, path = FigDir, height=PanelHeight, width=PanelWidth, bg="white")

#Temp and Deficit
legend <- grid_arrange_shared_legend(t,d,nrow=2,ncol=1,position="bottom")
g <- grid.arrange(legend,top = textGrob(paste0("Historical and future projections for ", SiteID),
                                        gp=gpar(fontface="bold", col="black", fontsize=20)))
ggsave("TavgF_D.in-Timeseries.png", g, path = FigDir, height=PanelHeight, width=PanelWidth, bg="white")

#Temp and Runoff
legend <- grid_arrange_shared_legend(t,r,nrow=2,ncol=1,position="bottom")
g <- grid.arrange(legend,top = textGrob(paste0("Historical and future projections for ", SiteID),
                                        gp=gpar(fontface="bold", col="black", fontsize=20)))
ggsave("TavgF_Runoff.in-Timeseries.png", g, path = FigDir, height=PanelHeight, width=PanelWidth, bg="white")

#Temp and SWE
legend <- grid_arrange_shared_legend(t,s,nrow=2,ncol=1,position="bottom")
g <- grid.arrange(legend,top = textGrob(paste0("Historical and future projections for ", SiteID),
                                        gp=gpar(fontface="bold", col="black", fontsize=20)))
ggsave("TavgF_SWEaccum.in-Timeseries.png", g, path = FigDir, height=PanelHeight, width=PanelWidth, bg="white")
